...
import com.amanraj.distributed_promt2prod.account_service.service.PaymentProcessor;
...
public class StripePaymentProcessor implements PaymentProcessor {
    ...
    public void handleWebhookEvent(String type, StripeObject stripeObject, Map<String, String> metadata) {
        ...
        switch (type) {
            ...
            case "customer.subscription.updated" ->
                    handleCustomerSubscriptionUpdated((Subscription) stripeObject); // when user cancels or upgrade
            case "customer.subscription.deleted" ->
                    handleCustomerSubscriptionDeleted((Subscription) stripeObject); // when subcription ends , revoke access
            case "handle.invoice.paid" -> {
                try {
            ...
            case "handle.invoice.payment_failed" ->
                    handleInvoicePaymentFailed((Invoice) stripeObject); // when invoice is not paid make it past_due
    ...
    private void handleCustomerSubscriptionUpdated(Subscription subscription) {
        if (subscription == null) {
            log.error("subscription was null inside handleCustomerSubscriptionUpdated ");
            return;
        }
        ...
        Instant perionStart = toInstant(item.getCurrentPeriodStart());
        Instant periodEnd = toInstant(item.getCurrentPeriodEnd());
        Long planId = resolvePlanId(item.getPrice());

        subscriptionService.updateSubscription(
                subscription.getId(), status, perionStart, periodEnd,
